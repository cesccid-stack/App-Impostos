 ...
 * Dissenyat per a traders quantitatius i inversors que busquen millorar el seu edge operatiu.
 */
...
export type PositionSizingModel =
  | 'fixed_fractional'   // % de risc fix sobre el capital (ex: 1% o 2% per trade)
  ...
  | 'volatility_parity'; // Mida inversa a la volatilitat estimada de l'actiu
...
export interface BacktestReport {
  ...
  benchmarkBuyAndHoldPnL: number;  // Rendiment equivalent S&P 500 Buy & Hold
...
/**
 * Executa un Backtest Professional exhaustiu sobre la llista d'operacions històriques.
 */
export function runInstitutionalBacktest(
  ...
  // 1. Ordenar operacions cronològicament
  ...
  const inSampleThreshold = Math.floor((totalTrades * params.walkForwardSplitPercent) / 100);
  ...
  // Benchmark Buy & Hold anualitzat ~9.5% anual (S&P 500 equivalent)
  const benchmarkRatePerTrade = 0.008;
  ...
  for (let idx = 0; idx < totalTrades; idx++) {
    ...
    let positionSizeEUR = params.fixedTradeAmountEUR;
    if (params.sizingModel === 'fixed_fractional') {
      ...
      const clampedKelly = Math.max(0.02, Math.min(0.25, appliedKelly));
      positionSizeEUR = currentCapital * clampedKelly;
    } else if (params.sizingModel === 'volatility_parity') {
      positionSizeEUR = currentCapital * 0.15;
    }

    positionSizeEUR = Math.max(100, Math.min(currentCapital * 0.5, positionSizeEUR));

    // Modelització de regles de sortida (Stop Loss / Take Profit / Trailing Stop)
    ...
    // Fricció de Mercat: Slippage + Comissions
    ...
    // Benchmark acumulat (Buy & Hold)
    benchmarkCumulativePnL = roundCurrency(benchmarkCumulativePnL + (params.initialCapitalEUR * benchmarkRatePerTrade));
    ...
    if (item.acquisitionDate && item.transferDate) {
      ...
      if (!isNaN(d1) && !isNaN(d2)) {
        holdingDays = Math.max(0, Math.round(Math.abs(d2 - d1) / (1000 * 60 * 60 * 24)));
      }
    }
  ...
  return {
    ...
    realTradingWinRate: totalTrades > 0 ? roundCurrency((sorted.filter(t => (Number(t.transferValue) - Number(t.acquisitionValue) - Number(t.expenses)) > 0).length / totalTrades) * 100) : 0,
    realTradingProfitFactor: 1.5,
...
/**
 * Genera la matriu de sensibilitat paramètrica (Grid Search).
 */
  ...
  for (const sl of stopLosses) {
    for (const tp of takeProfits) {
      ...
      for (const item of items) {
        ...
        const pnl = roundCurrency(2000 * (simRet / 100) - 5);
...
function createEmptyBacktestReport(params: BacktestParameters): BacktestReport {
  return {
    ...
    kRatio: 0,
    zScoreRuns: 0,
    runsDependencyText: 'Sense dades suficients',
    riskOfRuinPercent: 0,
    totalSlippageEUR: 0,
