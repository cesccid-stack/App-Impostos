...
export function renderTradingAnalytics(): HTMLElement {
  ...
  // Subscripció reactiva al store amb neteja en desmuntatge (desregistrament del listener per evitar fuites)
  const unsubscribe = store.subscribeKey('gains', () => render());
  router.registerCleanup(unsubscribe);
